...
const handleMessage = async (clinicConfig, fromNumber, msg) => {
    ...
    // Menu handling or FAQ/booking follow-up
    if (session.faqMode !== 'awaiting_continue') {
    ...
    if (session.faqMode === true) {
        ...
        await sendMessage(fromNumber, "Do you want to ask another question? (Yes/No)");
        session.faqMode = 'awaiting_continue';
        return;
    }
    ...
    if (session.faqMode === 'awaiting_continue') {
        if (msg.toLowerCase() === 'yes') {
            ...
            await sendMessage(fromNumber, `🙏 Thank you for visiting *${clinicConfig.clinic_name}*!`);
    ...
    switch (session.step) {
        ...
        case 8:
            ...
            session.step = 2; // keep session active for other questions
            break;
    }
};
